...
function run_issue_agent
    ...
    set -l command wt switch
    if local_branch_exists "$branch"
        set -a command "$branch"
    else
        set -a command -c "$branch"
        if test -n "$base"
            set -a command -b "$base"
        end
    end
    set -a command -x cx -- (agent_options "$issue_title") -- "$prompt"
    $command
end
...
function run_current_repo_issue_agent
    ...
    set -l command cx (agent_options "$issue_title") -- "$prompt"
    $command
end
...
function import_url
    ...
    set -l command cx (agent_options '') -- "$prompt"
    $command
end
...
function run_pr_agent
    ...
    set -l command wt switch "pr:$pr" -x cx -- (agent_options "$pr_title") -- "$prompt"
    $command
end
